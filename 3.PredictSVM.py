...
from Models.ParameterGrids import SVM_parameter_grid
import pandas as pd
...
def main () :
    ...
    for p in prevalence_rates:
        output_file = output_path+"SVM"+str(p)+".json"
        data = pd.read_csv(timeseries_path+"TimeSeries"+str(p)+".csv")
        X = data[features]
        y = data[outcome]

        svm_grid = SVM_parameter_grid()

        clf = svm.SVC(class_weight ='balanced',probability=True)
        grid_search = GridSearchCV(estimator=clf, param_grid=svm_grid,
